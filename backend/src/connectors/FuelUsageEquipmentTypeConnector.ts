...
import { FuelUsageEquipmentType } from '../entities/FuelUsageEquipmentType';

export class FuelUsageEquipmentTypeConnector extends BaseConnector {
  protected endpoint = '/consumo/tipo-equipamento';
  ...
  async normalize(rawData: any): Promise<any[]> {
    ...
    return dataArray.map((item: any) => ({
      external_id: String(item.id || item.tipo_id || `tipo_${Date.now()}_${Math.random()}`),
      type_code: item.codigo_tipo || item.codigo,
      type_name: item.nome_tipo || item.tipo_equipamento,
  ...
  async upsert(normalizedData: any[]): Promise<void> {
    const repository = AppDataSource.getRepository(FuelUsageEquipmentType);
