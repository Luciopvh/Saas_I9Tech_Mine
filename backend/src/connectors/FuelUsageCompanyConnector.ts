...
import { FuelUsageCompany } from '../entities/FuelUsageCompany';

export class FuelUsageCompanyConnector extends BaseConnector {
  protected endpoint = '/consumo/empresa';
  ...
  async normalize(rawData: any): Promise<any[]> {
    ...
    return dataArray.map((item: any) => ({
      external_id: String(item.id || item.empresa_id || `emp_${Date.now()}_${Math.random()}`),
      company_code: item.codigo_empresa || item.codigo,
      company_name: item.nome_empresa || item.empresa,
  ...
  async upsert(normalizedData: any[]): Promise<void> {
    const repository = AppDataSource.getRepository(FuelUsageCompany);
